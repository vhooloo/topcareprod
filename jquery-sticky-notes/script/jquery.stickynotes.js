(function(jQuery) {
	...
	jQuery.fn.stickyNotes.deletemyNote =  function(note_id) {
	    //alert("delete");
		jQuery.each(jQuery.fn.stickyNotes.notes, function(index, note) {
			if (note.id == note_id) {
				jQuery.fn.stickyNotes.notes.splice(index, 1);
				//alert(JSON.stringify(jQuery.fn.stickyNotes.notes));
				return false;
			}
		});
	}
	...
	jQuery.fn.stickyNotes.prepareContainer = function(container) {
		...
		//jQuery(container).prepend('<div id="sticky-container" class="sticky-container"></div>');
		if (jQuery.fn.stickyNotes.options.controls) {
			...
			//jQuery("#get_note").click(function() {
			//	alert(JSON.stringify(jQuery.fn.stickyNotes.notes));
			//	//jQuery.each(jQuery.fn.stickyNotes.notes, function(index, note) { alert(note.text); });
			//	return false;
			//});
		}
	...
	jQuery.fn.stickyNotes.createNote = function() {
		...
		var note = {"id":note_id,
		      "text":"",
			  "pos_x": jQuery("#note-"+note_id).position().left,
			  "pos_y": jQuery("#note-"+note_id).position().top,
			  //"pos_x": jQuery("#note-"+note_id).offset().left,
			  //"pos_y": jQuery("#note-"+note_id).offset().top,
			  //"pos_x": jQuery("#notes").offset().left,
			  //"pos_y": jQuery("#notes").offset().top,			  
	...
	jQuery.fn.stickyNotes.stopEditing = function(note_id) {
		...
		//}
		//alert("stopped edit");
		//$("#stickynotes_history_c").html(JSON.stringify(jQuery.fn.stickyNotes.notes));
	...
	jQuery.fn.stickyNotes.renderNote = function(note) {
		...
		var _div_delete = 	$(document.createElement('div'))
							...
							.click(function(){jQuery.fn.stickyNotes.deleteNote(this);});
		var _div_wrap 	= 	$(document.createElement('div'))
							.css({'position':'absolute','top':note.pos_y+'px','left':note.pos_x+'px', 'float': 'none',"width":note.width,"height":note.height})
		...
		_div_wrap.draggable({containment: '#notes', scroll: false, stop: function(event, ui){jQuery.fn.stickyNotes.movedNote(note.id)}}); 
	...
	jQuery.fn.stickyNotes.movedNote = function(note_id) {
		var note = jQuery.fn.stickyNotes.getNote(note_id);
		//alert( jQuery("#note-" + note_id).offset().left + "  " + jQuery("#note-" + note_id).offset().top);
		//note.pos_x=jQuery("#note-" + note_id).css("left").replace(/px/, "");
		//note.pos_y=jQuery("#note-" + note_id).css("top").replace(/px/, "");
		//note.pos_x=jQuery("#note-" + note_id).offset().left;
		//note.pos_y=jQuery("#note-" + note_id).offset().top;
		note.pos_x=jQuery("#note-" + note_id).position().left;
		note.pos_y=jQuery("#note-" + note_id).position().top;
    ...
    jQuery.fn.stickyNotes.createNoteBackground = function() {
        ...
        if (jQuery.browser.msie && jQuery.browser.version <= 6)  {
            background = $(document.createElement('div')).addClass("background").html('<img src="custom/topcarejs/jquery-sticky-notes/images/spacer.gif" class="stretch" style="margin-top:5px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'custom/topcarejs/jquery-sticky-notes/images/sticky-bg.png\',sizingMethod=\'scale\'");" alt="" />');
        } else {
            background = $(document.createElement('div')).addClass("background").html('<img src="custom/topcarejs/jquery-sticky-notes/images/sticky-bg.png" class="stretch" style="margin-top:5px;" alt="" />');
        }
        return background;
