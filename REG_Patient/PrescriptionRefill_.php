...
$metadataFile = $this->getMetaDataFile();
		...
		$query2a = "SELECT * FROM reg_encounter natural join reg_encounter_cstm where id=id_c and id in( SELECT  reg_patient_reg_encounterreg_encounter_idb from reg_patient_reg_encounter_c where reg_patient_reg_encounterreg_patient_ida = '".$this->bean->id."' AND deleted!=1 ) order by date_modified desc" ;

		$result = $this->bean->db->query($query2a, true); 
		
		if(($row = $this->bean->db->fetchByAssoc($result) ) != null )
		{
			if($row['last_rx_generated_c']!=null){
				$date1=strtotime($row['last_rx_generated_c']);
				echo "<script>document.getElementById('last_rx_generated_c').value='".date('m/d/Y',$date1)."'</script>";
			}
		 
			echo "<script>document.getElementById('last_rx_whom_c').value='".$row['last_rx_whom_c']."'</script>";
		 
			if($row['last_rx_signed_md_c']!=null){
				$date1=strtotime($row['last_rx_signed_md_c']);
				echo "<script>document.getElementById('last_rx_signed_md_c').value='".date('m/d/Y',$date1)."'</script>";
			}
			if($row['last_rx_pickup_pt_c']!=null){
				$date1=strtotime($row['last_rx_pickup_pt_c']);
				echo "<script>document.getElementById('last_rx_pickup_pt_c').value='".date('m/d/Y',$date1)."'</script>";
			}
			...
			if($row['nxt_appt_phy_c']!=null){
				$date1=strtotime($row['nxt_appt_phy_c']);
				echo "<script>document.getElementById('nxt_appt_phy_c').value='".date('m/d/Y',$date1)."'</script>";
				if(date('m/d/Y',$date1)<$datenow){
				  echo "<script>document.getElementById('nxt_appt_phy_c').style.color='red'</script>";
				}
			}
			if($row['nxt_appt_behav_c']!=null){
				$date1=strtotime($row['nxt_appt_behav_c']);
				echo "<script>document.getElementById('nxt_appt_behav_c').value='".date('m/d/Y',$date1)."'</script>";
				if(date('m/d/Y',$date1)<$datenow){
				  echo "<script>document.getElementById('nxt_appt_behav_c').style.color='red'</script>";
				}
			}
	...
	*/	
	...
	//echo "<h1 color='r\"e\"d'>hahhahah</h1>";
	 ...
	 if(empty($this->bean->id)){
        ...
        echo "<table width='100%'><tr><td  width='65%' style='border-color: rgb( 100, 100, 255); border-style: ridge;border-width: 2px;margin-top: 0;vertical-align: top;'  ><h1>Prescription Refill : ".$this->bean->name." &nbsp;&nbsp; <img src='images.jpg' width='12px'></img></h1>";
        $this->dv->process();
...
//			echo "<br/>";
		...
		//Alert Section
		echo "<td style='border-color: rgb( 100, 100, 255); border-style: ridge;border-width: 2px;vertical-align: top;' ><div id='accordion'><h3 style='color:red;font-size:16px;font-weight:bold;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Alerts</h3><div id='REG_Treatment_Plan_detailview_tabs'><div id='detailpanel_1' class='detail view  detail508 '><table id='DEFAULT'></table><table  class='panelContainer' cellspacing='0' border='1'><tbody><tr>
...
//			echo "<br/>";
				...
				if($count%2==1){
					echo "<tr height=\"20\" class=\"oddListRowS1\"><td scope=\"row\" valign=\"top\" class=\"\"><span sugar=\"slot1b\"><a href=\"?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3DREG_Encounter%26action%3DDetailView%26record%3D".$row['id']."\">".$row['summary']."</a></span></td><td scope=\"row\" valign=\"top\" class=\"\"><span sugar=\"slot2b\">".$date_created[0]."</span></td><td scope=\"row\" valign=\"top\" class=\"\"><span sugar=\"slot2b\">".$row2['first_name']."&nbsp;".$row2['last_name']."</span></td></tr>";
				}
				else
				{
					echo "<tr height=\"20\" class=\"evenListRowS1\"><td scope=\"row\" valign=\"top\" class=\"\"><span sugar=\"slot1b\"><a href=\"?action=ajaxui#ajaxUILoc=index.php%3Fmodule%3DREG_Encounter%26action%3DDetailView%26record%3d".$row['id']."\">".$row['summary']."</a></span></td><td scope=\"row\" valign=\"top\" class=\"\"><span sugar=\"slot2b\">".$date_created[0]."</span></td><td scope=\"row\" valign=\"top\" class=\"\"><span sugar=\"slot2b\">".$row2['first_name']."&nbsp;".$row2['last_name']."</span></td></tr>";
				}
